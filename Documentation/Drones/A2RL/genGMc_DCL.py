...
# estimate compared to our own... frame same weight, but VTX on it. Also carrier board heavier
m = 0.900 # kg

# OUTWARDS propellers are most likely. todo: check this
...
# Option 2: direct: guess a little more than our own in pitch and yaw, but roll the same (orin isnt ontop in DCL)
Ixx = 0.001441*1.25
Iyy = 0.001551*1.75
Izz = 0.001772*1.75
...
# https://www.flyingtech.co.uk/product/gemfan-hurricane-51466-v2-props/
mp = 4.2 * 1e-3 # 1% error: 1% error in inertia --> estimated precision 0.02g, so 1.5% error
...
tau = 0.03 # spinup/spindown time constant
Tmax = 20. # estimate for 51466 props: https://www.xnovamotors.com/new-xnova-black-thunder-2207-2100kv-racing-motor/
k = Tmax / (30000 / 60. * 2 * np.pi)**2 # constant in T = k omega^2 -- estimate using https://www.xnovamotors.com/new-xnova-black-thunder-2207-2100kv-racing-motor/
...
diagonal = np.hypot(widthRear, length)
...
if (Ixx and Iyy and Izz):
    ...
    Ixx = inertiaFromPendulumPeriod(Px, Rx, m - (4*mp)) + (4*mp) * (widthRear/2)**2
